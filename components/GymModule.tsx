...
export const GymModule: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col space-y-6 relative">
      ...
      {showCreateWorkoutModal && (<div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center backdrop-blur-sm p-4 animate-in fade-in"><div className="bg-white rounded-2xl w-full max-w-3xl shadow-2xl max-h-[90vh] flex flex-col"><div className="p-6 border-b border-gray-100 flex justify-between items-center"><h3 className="text-2xl font-bold text-brand-slate">Programar treino</h3><button onClick={() => setShowCreateWorkoutModal(false)} className="p-2 hover:bg-gray-100 rounded-full text-gray-400"><X size={24} /></button></div><div className="p-6 overflow-y-auto flex-1 space-y-6"><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="text-xs font-bold text-gray-500 mb-1 block">Nome do treino</label><input type="text" value={formTitle} onChange={(e) => setFormTitle(e.target.value)} className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:border-brand-orange outline-none"/></div><div><label className="text-xs font-bold text-gray-500 mb-1 block">Categoria</label><select value={formCategory} onChange={(e) => setFormCategory(e.target.value)} className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:border-brand-orange outline-none"><option>Geral</option><option>Infantil</option><option>Petiz</option></select></div></div><div><label className="text-xs font-bold text-gray-500 mb-1 block">Data</label><input type="date" value={formDate} onChange={(e) => setFormDate(e.target.value)} className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:border-brand-orange outline-none"/></div><div><label className="text-xs font-bold text-gray-500 mb-1 block">Modelo (opcional)</label><select value={formBaseTemplateId} onChange={(e) => handleBaseTemplateChange(e.target.value)} className="w-full p-2.5 bg-white border border-gray-300 rounded-lg focus:border-brand-orange outline-none text-brand-slate"><option value="">Sem modelo...</option>{templates.map(t => <option key={t.id} value={t.id}>{t.title}</option>)}</select></div><div className="border-t border-gray-100 pt-6"><h4 className="text-sm font-black text-brand-slate mb-5 tracking-widest flex items-center gap-2"><Dumbbell size={18} /> Exercícios</h4><ExerciseFormList /></div></div><div className="p-6 border-t border-gray-100 bg-gray-50 rounded-b-2xl flex justify-end gap-3"><button onClick={() => setShowCreateWorkoutModal(false)} className="px-6 py-2.5 text-gray-600 font-bold hover:bg-white rounded-xl transition-colors">Cancelar</button><button onClick={saveNewWorkout} className="px-8 py-2.5 bg-brand-orange text-white font-black uppercase text-xs tracking-widest rounded-xl shadow-lg shadow-orange-500/20 hover:bg-orange-600 transition-all">Salvar treino</button></div></div></div>)}
