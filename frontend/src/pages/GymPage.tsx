...
const FilterChip: React.FC<{ label: string, active: boolean, onClick: () => void }> = ({ label, active, onClick }) => (
  <button
    onClick={onClick}
    className={`px-4 py-2 rounded-xl text-[10px] font-black transition-all tracking-widest border border-transparent whitespace-nowrap ${active ? 'bg-brand-orange text-white shadow-md' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}`}
  >
    {label}
  </button>
);
      ...
      <div className="space-y-6 animate-in fade-in">
        <div className="bg-white p-4 md:p-6 rounded-[32px] border border-slate-200 shadow-sm flex flex-col gap-4 md:gap-6">
          <div className="flex items-center justify-between md:hidden">
            ...
            <button
              onClick={() => setShowAgendaFilters(!showAgendaFilters)}
              className="p-2 bg-slate-50 rounded-xl text-slate-400 hover:text-brand-orange transition-all"
            >
              {showAgendaFilters ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
            </button>
          </div>

          <div className={`${showAgendaFilters ? 'flex' : 'hidden md:flex'} flex-col gap-6 animate-in slide-in-from-top-2`}>
        ...
        <div className="bg-white p-4 md:p-6 rounded-[32px] border border-slate-200 shadow-sm flex flex-col gap-4 md:gap-6">
          <div className="flex items-center justify-between md:hidden">
            ...
            <button
              ...
              className="p-2 bg-slate-50 rounded-xl text-slate-400 hover:text-brand-orange transition-all"
            >
              {showLibraryFilters ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
            </button>
          </div>

          <div className={`${showLibraryFilters ? 'flex' : 'hidden md:flex'} flex-col gap-6 animate-in slide-in-from-top-2`}>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Search */}
              <div className="flex flex-col gap-3">
                <span className="text-[10px] text-slate-400 uppercase font-black tracking-widest pl-1">Buscar:</span>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:border-brand-orange focus:bg-white outline-none transition-all font-bold text-slate-700"
                  />
                </div>
              </div>
