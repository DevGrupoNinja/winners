...
export default function AnalyticsPage() {
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-8 pb-10">
                    ...
                    <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm p-8">
                        <div className="flex items-center justify-between mb-8">
                            ...
                            <div className="flex flex-wrap gap-2 p-1 bg-slate-50 rounded-[20px] border border-slate-100">
                                {technicalCategories.map(cat => (
                                    <button
                                        key={cat.id}
                                        onClick={() => setSelectedTechnical(cat.id as TechnicalCategory)}
                                        className={`px-4 py-2.5 rounded-[14px] text-[9px] font-black transition-all uppercase tracking-widest ${selectedTechnical === cat.id ? 'bg-brand-slate text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}
                                    >
                                        {cat.label}
                                    </button>
                                ))}
    ...
    const renderListView = () => (
        <div className="h-full flex flex-col space-y-6 animate-in fade-in duration-500 overflow-hidden pb-10">
            ...
            <div className="flex flex-wrap items-center gap-2 self-start animate-in slide-in-from-top-2 py-2">
                {technicalCategories.map((cat) => (
                    <button
                        key={cat.id}
                        onClick={() => setSelectedTechnical(cat.id as TechnicalCategory)}
                        className={`flex items-center gap-2 px-5 py-3 rounded-2xl text-[10px] font-black transition-all uppercase tracking-widest border border-transparent ${selectedTechnical === cat.id
                            ? 'bg-brand-slate text-white shadow-lg'
                            : 'bg-white text-slate-400 border-slate-100 hover:border-slate-200 shadow-sm'
                            }`}
                    >
                        <cat.icon size={16} className={selectedTechnical === cat.id ? 'text-brand-orange' : 'text-slate-300'} />
                        {cat.label}
                    </button>
                ))}
            </div>
            ...
            <div className="bg-white rounded-[32px] border border-slate-200 shadow-sm p-4 md:p-6 flex flex-col gap-4 md:gap-6 animate-in slide-in-from-top-4">
                ...
                <div className="relative">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 focus-within:text-brand-orange transition-colors font-bold" size={18} />
                    <input
                        type="text"
                ...
                <div className={`${showFilters ? 'flex' : 'hidden md:flex'} flex-col md:flex-row md:items-center gap-6 animate-in slide-in-from-top-2`}>
                    <div className="flex flex-col md:flex-row md:items-center gap-4">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">Categoria:</span>
                        <div className="flex flex-wrap items-center gap-1">
                            {categories.map((group) => (
                                <button
                                    key={group}
                                    onClick={() => setSelectedGroup(group)}
                                    className={`px-5 py-2.5 rounded-xl text-[10px] font-black transition-all border border-transparent uppercase tracking-widest whitespace-nowrap ${selectedGroup === group
